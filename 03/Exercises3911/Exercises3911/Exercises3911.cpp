// Exercises3911.cpp : This file contains the 'main' function. Program execution begins and ends there.
//

// using erfc(x) = 1- erf(x) included in <math.h>
// erf(x) = 2 / sqrt(pi) * integrate(exp(-t^2)) from 0 to x
...
int main()
{
    ...
    cout << endl;
    cout << "Test normcdf using erfc(x)" << endl;
    double x = 0.5;
    double y = -0.65;
    cout << normcdf_(x) << endl;
    cout << normcdf(x) << endl;
    cout << normcdf_(y) << endl;
    cout << normcdf(y) << endl;
    cout << "Test end" << endl;
    ...
    double volatility, double riskFreeInterestRate)
{
    ...
    double putPrice = normcdf(-d2) * strikePrice * exp(-riskFreeInterestRate * maturity)
        - normcdf(-d1) * spotPrice;
    return putPrice;
}
